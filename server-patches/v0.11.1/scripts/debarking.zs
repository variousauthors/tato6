...
val strippedLogs = [
  ...
  // <debark:debarked_log_roots_wildwood_log>, // have to add this later, if we add a stripped form
] as IItemStack[];
...
events.onPlayerInteractBlock(function (event as crafttweaker.event.PlayerInteractBlockEvent) {
  ...
  if (id != 'minecraft:log' && id != 'minecraft:log2') {
    return;
  }

  val defaultBlock = event.block.definition.defaultState.block;
  val eventVariant = event.block.definition.getStateFromMeta(event.block.meta).getPropertyValue('variant');

  for i, log in logs {
    val logVariant = log.asBlock().definition.getStateFromMeta(log.asBlock().meta).getPropertyValue('variant');

    if (logVariant == eventVariant) {
      ...
      if (toolClasses has axe) {
        ...
        val state = event.block.definition.getStateFromMeta(event.block.meta);
        val axis = state.getPropertyValue('axis');
        event.world.setBlockState(
          strippedLog.asBlock().definition.defaultState.withProperty('axis', axis), 
          event.position
        );
        break;
      }
